...
public class WordMatch {
    ...
    void readFile(String fileName) {
        try {
            ...
            while (fileReader.hasNext()) {
                ...
                fileLine = fileReader.nextLine().toLowerCase().replaceAll("--", " ").replaceAll("[^a-z\\s]", "")
                        .replaceAll("\\s+", " ").trim();
            ...
            sort(rawData, 0, rawData.size() - 1, false);
            fileReader.close();
        } catch (IOException e) {
        ...
        try {
            ...
            for (int i = 0; i < lexicon.size(); i++) {
                fileLine += lexicon.get(i) + " " + numberOfOccurances.get(i) + " "
                        + getNeighbours(lexiconClone, lexicon.get(i)).toString() + "\n";
            }
    ...
    /**
     * Function to count the number of occurances of a word in the lexicon, and to
     * remove the duplicate words from lexicon.
     */
    private void createLexicon() {
        Integer count = 0;
        for (int i = 0; i < rawData.size(); i++) {
            lexicon.add(rawData.get(i));
            for (int j = i; j < rawData.size(); j++) {
                if (rawData.get(i).equals(rawData.get(j))) {
                    count++;
                } else {
                    i = j - 1;
                    break;
    ...
    /**
     * Function to compile the pattern into regular expression eg. ?a*t* =
     * (.?)a(.*)t(.*)
     * 
     * @param pattern
     * @return
    ...
    ArrayList<String> getNeighbours(ArrayList<String> lexiconClone, String word) {
        ...
        for (int j = 0; j < lexiconClone.size(); j++) {
            if (word.equals(lexiconClone.get(j))) {
                continue;
            }
            if (lexiconClone.get(j).length() > word.length()) {
                break;
            }
            if (lexiconClone.get(j).length() < word.length()) {
                continue;
            ...
            for (int j2 = 0; j2 < word.length(); j2++) {
                if (word.charAt(j2) != lexiconClone.get(j).charAt(j2)) {
                    missmatch += 1;
                }
            }
            if (missmatch == 1) {
     ...
     * @param low
     * @param high
     * @param byLength Parameter "byLength" is used to chose the type of sort i.e.
     *                 sort by alphaphets or sort by word length byLength=true means
     *                 sort by length byLength=false means sort by alphabets
     */
